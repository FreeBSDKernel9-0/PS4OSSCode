...
{
    { "postMessage", DontDelete | JSC::Function, (intptr_t)static_cast<NativeFunction>(jsDedicatedWorkerContextPrototypeFunctionPostMessage), (intptr_t)1, NoIntrinsic },
    { 0, 0, 0, 0, NoIntrinsic }
};
...
{
    return getHashTableForGlobalData(exec->vm(), &JSDedicatedWorkerContextPrototypeTable);
}
...
{
    return getHashTableForGlobalData(exec->vm(), &JSDedicatedWorkerContextTable);
}
...
JSDedicatedWorkerContext::JSDedicatedWorkerContext(VM& vm, Structure* structure, PassRefPtr<DedicatedWorkerContext> impl)
    : JSWorkerContext(vm, structure, impl)
{
}
