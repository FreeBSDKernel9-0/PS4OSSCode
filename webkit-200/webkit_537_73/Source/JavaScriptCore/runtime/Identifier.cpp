...
struct IdentifierASCIIStringTranslator {
    ...
    static void translate(StringImpl*& location, const LChar* c, unsigned hash)
    {
        ...
        location = StringImpl::create(c, length, StringImpl::StringImplShared).leakRef();
#else
...
PassRefPtr<StringImpl> Identifier::add(VM* vm, const char* c)
{
    ...
    IdentifierTableHashSet::AddResult addResult = identifierTable.add<const LChar*, IdentifierASCIIStringTranslator>(reinterpret_cast<const LChar*>(c));
...
PassRefPtr<StringImpl> Identifier::add8(VM* vm, const UChar* s, int length)
{
    ...
    IdentifierTableHashSet::AddResult addResult = vm->identifierTable->add<CharBuffer<UChar>, IdentifierLCharFromUCharTranslator >(buf);
