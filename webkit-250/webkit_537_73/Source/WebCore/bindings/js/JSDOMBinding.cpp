...
#include <bindings/js/ScriptValue.h>
...
void reportException(ExecState* exec, JSValue exception, CachedScript* cachedScript)
{
    ...
    scriptExecutionContext->reportException(errorMessage, lineNumber, columnNumber, exceptionSourceURL, ScriptValue(exec->vm(), exception), callStack->size() ? callStack : 0, cachedScript);
}
