...
namespace JSC {
    ...
    public:
        ...
        Identifier(EmptyIdentifierFlag) : m_string(StringImpl::empty_shared()) { }
        ...
        Identifier(ExecState* exec, StringImpl* rep) : m_string(add_shared(exec, rep)) { }
        Identifier(ExecState* exec, const String& s) : m_string(add_shared(exec, s.impl())) { }
        ...
        Identifier(VM* vm, StringImpl* rep) : m_string(add_shared(vm, rep))
        {
#if ENABLE(DETACHED_JIT)
            RELEASE_ASSERT(JITSharedDataMemory::isInRange((uintptr_t)rep));
#endif
        } 
        Identifier(VM* vm, const String& s) : m_string(add_shared(vm, s.impl())) { }
    ...
    private:
        ...
        template <typename T>
        static PassRefPtr<StringImpl> add_shared(T* t, StringImpl* r)
        {
#if ENABLE(DETACHED_JIT)
            if (JITSharedDataMemory::isInRange(reinterpret_cast<uintptr_t>(r))) {
                return add(t, r);
            }

            RefPtr<StringImpl> sharedString = r->is8Bit() ?
                StringImpl::create(r->characters8(), r->length(), StringImpl::StringImplShared) :
                StringImpl::create(r->characters16(), r->length(), StringImpl::StringImplShared);
            //sharedString->setIsIdentifier(r->isIdentifier());
            //sharedString->setIsAtomic(r->isAtomic());
            return add(t, sharedString.get());
#else
            return add(t, r);
#endif
        }
    ...
    struct IdentifierCharBufferTranslator {
        ...
        {
            T* d;
            StringImpl* r = StringImpl::createUninitialized(buf.length, d, StringImpl::StringImplShared).leakRef();
    ...
    PassRefPtr<StringImpl> Identifier::add(VM* vm, const T* s, int length)
    {
        ...
        IdentifierTableHashSet::AddResult addResult = vm->identifierTable->add<CharBuffer<T>, IdentifierCharBufferTranslator<T> >(buf);
    ...
    IdentifierTableHashSet::AddResult IdentifierTable::add(U value)
    {
        IdentifierTableHashSet::AddResult result = m_table.add<U, V>(value);
