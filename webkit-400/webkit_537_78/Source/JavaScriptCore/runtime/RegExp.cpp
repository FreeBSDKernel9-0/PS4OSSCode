...
RegExp::RegExp(VM& vm, const String& patternString, RegExpFlags flags)
    ...
    , m_regExpJITCode_shared(adoptPtr(new Yarr::YarrCodeBlock()))
    , m_regExpJITCode(*m_regExpJITCode_shared)
#endif
{
}
...
#if ENABLE(DETACHED_JIT)
    OwnPtr<Yarr::YarrPattern> pattern_shared(adoptPtr(sharedNew<Yarr::YarrPattern>(m_patternString, ignoreCase(), multiline(), &m_constructionError)));
    Yarr::YarrPattern & pattern(*pattern_shared);
#else
