...
# the -x option in jsc.cpp
        ...
        close (OUTPUT);
            ...
            mkdir ("results");
            @ENV{ORBIS_JSC_CONSOLE_ARGS} = @ENV{ORBIS_JSC_CONSOLE_ARGS} . " --batch=" . $opt_batch_list;
            @ENV{PSP2_JSC_CONSOLE_ARGS} = @ENV{PSP2_JSC_CONSOLE_ARGS} . " --batch=" . $opt_batch_list;
            system(&xp_path($engine_command));
            return;
