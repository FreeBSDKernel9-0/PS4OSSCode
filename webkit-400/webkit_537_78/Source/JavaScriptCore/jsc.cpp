...
static EncodedJSValue JSC_HOST_CALL functionToggleJit(ExecState*);
...
#endif
        addFunction(vm, "toggleJit", functionToggleJit, 1);
...
EncodedJSValue JSC_HOST_CALL functionToggleJit(ExecState* exec)
{
    char const * const jitSettingKey_baseline = "JSCORE_ENABLE_JIT";
    char const * const jitSettingKey_dfg = "JSCORE_ENABLE_DFG";

    int const jitLevel = exec->argument(0).toInt32(exec);
    switch (jitLevel) {
    case 0:
        setenv(jitSettingKey_baseline, "0", 1);
        setenv(jitSettingKey_dfg, "0", 1);
        break;
    case 1:
        setenv(jitSettingKey_baseline, "1", 1);
        setenv(jitSettingKey_dfg, "0", 1);
        break;
    case 2:
        setenv(jitSettingKey_baseline, "1", 1);
        setenv(jitSettingKey_dfg, "1", 1);
        break;
    default:
        return throwVMError(exec, createError(exec, "Invalid JIT level."));
    }
