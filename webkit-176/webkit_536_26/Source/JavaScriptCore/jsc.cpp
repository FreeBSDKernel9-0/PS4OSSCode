...
static EncodedJSValue JSC_HOST_CALL functionBridgeProfilerStart(ExecState*);
static EncodedJSValue JSC_HOST_CALL functionBridgeProfilerStop(ExecState*);
#endif
...
#endif
#if ENABLE(BRIDGE_PROFILER)
        addFunction(globalData, "bridgeProfilerStart", functionBridgeProfilerStart, 1);
        addFunction(globalData, "bridgeProfilerStop", functionBridgeProfilerStop, 1);
#endif
...
#if ENABLE(BRIDGE_PROFILER)
EncodedJSValue JSC_HOST_CALL functionBridgeProfilerStart(ExecState*)
{
    JSCBridge::Profiler::enable(true);
    return JSValue::encode(jsBoolean(true));
}

EncodedJSValue JSC_HOST_CALL functionBridgeProfilerStop(ExecState*)
{
    JSCBridge::Profiler::enable(false);
    return JSValue::encode(jsNumber(JSCBridge::Profiler::total()));
}
#endif // ENABLE(JSCBRIDGE_PROFILING)
