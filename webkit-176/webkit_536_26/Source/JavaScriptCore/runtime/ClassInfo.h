...
#define CREATE_METHOD_TABLE(ClassName) { \
    ...
    struct ClassInfo {
        ...
        bool isSubClassOf(const ClassInfo* other) const
        {
            ...
            ClassInfo const * me(JSCBridge::sharedInstance()->mapFromVMToCompiler(this));
            other = JSCBridge::sharedInstance()->mapFromVMToCompiler(other);
            for (const ClassInfo* ci = me; ci; ci = ci->parentClass) {
                ci = ci ? JSCBridge::sharedInstance()->mapFromVMToCompiler(ci) : ci;
                if (ci == other)
                    return true;
            }
            return false;
#else
            ClassInfo const * me(this);
            for (const ClassInfo* ci = me; ci; ci = ci->parentClass) {
                if (ci == other)
                    return true;
            }
