...
#if (OS(DARWIN) || USE(PTHREADS) && !(OS(PSP2) || OS(PS3)))
...
#if OS(ORBIS)
static void deleteJSLockCount()
{
    pthread_key_delete(JSLockCount);
}
#endif
...
{
#if OS(ORBIS)
    if (!pthread_key_create(&JSLockCount, 0))
        atexit(deleteJSLockCount);
#else
