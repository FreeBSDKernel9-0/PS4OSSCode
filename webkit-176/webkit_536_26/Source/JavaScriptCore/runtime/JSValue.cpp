...
#if ENABLE(DETACHED_JIT) && !BUILDING_DETACHED_JIT
// This is mainly provided so that we can distinguish the function toInt32() that
// is defined in the VM process from that in the compiler process.  All compiler
// operation that normally links to toInt32 should really link against toInt32_vmstub
// to ensure the JIT'ed code properly calls into a valid function in the VM address space
int32_t toInt32_vmstub(double number)
{
