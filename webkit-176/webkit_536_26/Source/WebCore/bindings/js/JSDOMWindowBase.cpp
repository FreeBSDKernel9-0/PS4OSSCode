...
#if PLATFORM(MANX)
JSGlobalData* JSDOMWindowBase::s_globalData = NULL;
void JSDOMWindowBase::cleanup()
{
    if (s_globalData) {
        s_globalData->heap.collectAllGarbage();
        s_globalData->deref();
        s_globalData = NULL;
    }
}
...
#else
    JSGlobalData*& globalData = s_globalData;
#endif
    if (!globalData) {
        globalData = JSGlobalData::createLeaked(ThreadStackTypeLarge, LargeHeap).leakRef();
#if PLATFORM(MANX)
        globalData->timeoutChecker.setTimeoutInterval(3000); // 3 seconds
#else
