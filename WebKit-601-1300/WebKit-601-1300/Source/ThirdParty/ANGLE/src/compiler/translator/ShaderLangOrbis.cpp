...
namespace
{
    ...
    bool slPopulateValues(shader_t* shader)
    {
        ...
        // Load program
        Manx::ShaderBinary::Program* program = Manx::ShaderBinary::Program::create();

        if (!program || program->loadFromMemory(entry->code, entry->len)) {
            return false;
        }

        // Uniforms
        for (auto i = 0; i < program->numConstants(); ++i) {
            sh::Uniform mappedUniform;

            // Name is not mapped from one value to another
            std::string name(program->getNameFromConstantsAt(i));
        ...
        // Attributes
        for (auto i = 0; i < program->numInputAttributes(); ++i) {
            sh::Attribute mappedAttribute;

            // Name is not mapped from one value to another
            std::string name(program->getNameFromAttributesAt(i));
